...
{
  assert(img != nullptr); // checks if ptr is null

  // checks if the width and height are in bounds
...
void Image_init(Image *img, std::istream &is)
{
  ...
  is >> ppmType >> width >> height >> MAX_INTENSITY;

  Image_init(img, width, height); 
  //^ Initializing the matrices and setting width and height of the image

  for (int row = 0; row < Matrix_height(&img->red_channel); ++row)
  {
    for (int column = 0; column < Matrix_width(&img->red_channel); ++column)
    {
      int r;
      int g;
      int b;
...
{
  assert(img != nullptr); // checks if ptr is null
  return Matrix_width(&img->red_channel);
}
...
{
  assert(img != nullptr); // checks if ptr is null
  return Matrix_height(&img->red_channel);
}
